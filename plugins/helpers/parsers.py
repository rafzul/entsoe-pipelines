...
def parse_datetimeindex(spark, df_ts, df_nonts, tz=None):
    ...
    date_index = spark.sql(
        f"SELECT sequence(to_timestamp('{start}'), to_timestamp('{end}'), {delta}) as measured_at"
    ).withColumn("measured_at", F.explode(F.col("measured_at")))
    # if tz is not None:
    ...
    date_index = date_index.selectExpr(
        "measured_at",
        f"to_timestamp('{createdDateTime}') as created_at",
        "ROW_NUMBER() OVER (PARTITION BY '1' ORDER BY measured_at) AS position",
    )
